...
with st.expander("📖 Guía de uso y sintaxis matemática (Haz clic para expandir/contraer)", expanded=False):
    ...
    * **Exponencial y Logaritmos:** Usa `exp(x1)`, `ln(x1)`, `log(x1, 10)`.
    """)
...
CASOS = {
    ...
    "🟡 Valle de Rosenbrock (2 vars)": {
        "desc": "El 'Valle de la Banana'. Mínimo global en (1,1). Difícil convergencia.",
    ...
    while k < max_iter and historial_error[-1] > tol:
        ...
        if metodo_nombre == "Gradiente":
            ...
            try:
                ...
                if np.dot(pk, gk) >= 0: pk = -gk 
            except np.linalg.LinAlgError: pk = -gk 
...
if ejecutar:
    if modo_carrera:
        ...
        with c_num:
            ...
            # --- BITÁCORA Y CONTEXTO IA ---
            with st.expander("🤖 Generar Bitácora y Contexto IA", expanded=False):
                st.markdown("Copia este reporte para analizarlo con herramientas de Agentic AI (ChatGPT, Claude, Gemini):")
...
- Punto mínimo encontrado x*: {np.round(res['x'], 6).tolist()}
- Valor mínimo f(x*): {round(f_eval(res['x']), 6)}
- Norma del gradiente final: {res['error']:.2e}
- Iteraciones requeridas: {res['k']}
- Criterio de parada: {res['criterio_parada']}

**4. Solicitud para el Agente IA:**
Actúa como un experto en matemáticas aplicadas. Analiza esta bitácora y entrégame:
1. Una evaluación sobre la eficiencia del método {metodo} para esta función específica.
2. Si hubo problemas de convergencia, explica las posibles causas topográficas de la función.
3. Sugerencias de ajustes en los parámetros de Wolfe o en el punto de partida para mejorar el rendimiento."""
        ...
        if num_vars == 2 and res["k"] > 0:
            ...
            grid_res = 130
            X1, X2 = np.meshgrid(np.linspace(x_final[0] - margen, x_final[0] + margen, grid_res), np.linspace(x_final[1] - margen, x_final[1] + margen, grid_res))
            ...
            try:
                ...
                fig3d.add_trace(go.Scatter3d(x=tray[:, 0], y=tray[:, 1], z=np.array([f_eval(p) for p in tray]), mode="lines+markers", line=dict(color="#ffffff", width=4), marker=dict(size=3, color="#ffffff"), name="Camino"))
                ...
                fig3d.update_layout(template="plotly_dark", height=650, margin=dict(t=0, b=0, l=0, r=0), scene=dict(xaxis_title="x1", yaxis_title="x2", zaxis_title="f(x)", bgcolor="rgba(0,0,0,0)", camera=dict(eye=dict(x=1.5, y=1.5, z=1.2))), legend=dict(bgcolor="rgba(0,0,0,0.6)", yanchor="top", y=0.95, xanchor="left", x=0.05))
